...
validateMove b [i, j] = mj >>= (\j' ->
  if all inBounds [i', j'] && isEmpty (i', j')
    then Just (i', j') -- return a valid position if all validation predicates are satisfied
    else Nothing)
  where
...
diagMate t rows = all (== t) (trace rows) || all (== t) (antitrace rows)
  where
